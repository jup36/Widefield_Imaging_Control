function auditory_conditioning_operant_day4_singleDui_AO(app)
...
%Motif expression criteria variables (NEW Adi 04/29)
stimopts.neural_criterion_met = false(N * stimopts.NTrials_per_sequence, 1);
stimopts.motif_scores = cell(N * stimopts.NTrials_per_sequence, 1);
...
%% Start background motif classification (NEW 04/29 Adi)
% open a single background worker
p = gcp('nocreate');
if isempty(p)
    parpool(1);
end

% launch RealTimeMotifPredict in the background to continuously poll the
% shared drive TIFFs and write motif_result.mat
tif_path = 'Z:\Users\Adi\Recordings';
output_path = 'Z:\Users\Adi\MotifPredictions';
target_motif = 3;
score_threshold = 0.5;

load('motif_basis.mat', 'W_basis', 'nanpxs', 'gp');
future = parfeval(@RealTimeMotifPredict, 0, ...
    tif_path, W_basis, nanpxs, gp, output_path);

fprintf('Background motif classification started\n');
...
%             WaitSecs(stimopts.post_stim_delay_padding(index));
        % NEW (Adi 04/29)
        % outcome delivery wtih additional motif criteria included
        if stimopts.rewarded_stim(index)
            write(dui, "R", "char");
            fprintf('Rewarded trial\n');
            neural_criterion_met = false;
            reward_window_start = GetSecs();
            motif_scores_this_trial = [];
            % Poll neural classification during reward window
            while (GetSecs() - reward_window_start) < stimopts.reward_duration
                [metric, top_motif] = compute_neural_metric(...
                    output_path, target_motif, score_threshold);
                motif_scores_this_trial(end+1) = metric;        
                if top_motif == target_motif && metric > score_threshold ...
                        && ~neural_criterion_met
                    write(dui, "M", "char");  % upgrade to 2-pulse
                    neural_criterion_met = true;
                    fprintf('Trial %d: motif %d criterion met (score %.3f)\n', ...
                        index, top_motif, metric);
                end        
                drawnow;
                WaitSecs(0.050);
            end
            % Save neural result for this trial
            stimopts.neural_criterion_met(index) = neural_criterion_met;
            stimopts.motif_scores{index} = motif_scores_this_trial;
            write(dui, "I", "char");
            WaitSecs(stimopts.post_reward);
            write(dui, "S", "char");
            WaitSecs(stimopts.post_stim_delay_padding(index));       
            fprintf('Trial %d: neural criterion %s\n', index, mat2str(neural_criterion_met));
...
% NEW (Adi 04/29)
% close parpool at the end of the session
cancel(future);
delete(gcp('nocreate'));
fprintf('Background classification stopped\n');
...
% NEW (Adi 04/29)
function [metric, top_motif] = compute_neural_metric(result_path, target_motif, score_threshold)
    metric = 0;
    top_motif = NaN;
    ...
    % Check file is recent (within 2 seconds)
    info = dir(result_path);
    file_age = posixtime(datetime('now')) - posixtime(datetime(info.date));
    if file_age > 2.0
        return
    end

    try
        data = load(result_path, 'result');
        top_motif = data.result.top_motif;
        motif_act = data.result.motif_act;
        metric = motif_act(target_motif);
    catch
        return
    end
end
