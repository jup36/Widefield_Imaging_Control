function visual_reward_passive(app)
...
%Reward and two visual stimuli, one common (80%) and one uncommon (20%),
...
stim_sequence(end+1:end+stimopts.sequence_length)=3; %add 3 for reward only (no stim)

%now create N sequences that are shuffles of stim_squence
stim_type = [];
for i=1:N
...
%camera
total_duration_sequence=stimopts.sequence_length*(sti  mopts.baseline*2+stimopts.stim_duration+stimopts.post_reward);% we have n stim and n rewards, so n * baseline+stim + n*baseline+post_reward
behav_camera_pulse_dur=ceil(total_duration_sequence);
...
fprintf('Suc   esssfully completed trials info recording\n')
...
for i = 1:N %N is the number of sequences, each made of 10 rewards and 10 stim
    ...
    signal_out = white_noise_only(nidq.Rate, nidq_out_list, behav_camera_pulse_dur+stimopts.tail_camera_frame_padding+2);
    ...
    for j=1:stimopts.sequence_length*2
        index=(i-1)*stimopts.sequence_length*2 + j; %index in the stim_type vector
        ...
        if stimopts.stim_type(index)<3 %if it's a stim we show
            ...
            start_of_showGrating_conditioning(opts,stimopts.stim_type(index));
