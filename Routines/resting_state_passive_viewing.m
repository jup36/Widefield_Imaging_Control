function resting_state_passive_viewing(app)
...
%camera
total_duration_sequence=stimopts.sequence_length*(stimopts.baseline+stimopts.stim_duration);% we have n stim and n rewards, so n * baseline+stim + n*baseline+post_reward
...
for i = 1:N %N is the number of sequences, each made of 10 rewards and 10 stim
    ...
    digital_out(nidq, nidq_out_list, "trigger", 10/round(app.cur_routine_vals.framerate), 4)
    tic
    % Play white noise while launching beh camera
    signal_out = white_noise_only(nidq.Rate, nidq_out_list, behav_camera_pulse_dur+stimopts.tail_camera_frame_padding);
