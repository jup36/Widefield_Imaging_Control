function visual_conditioning_with_bhv_cam_tbytVideo(app)
...
stimopts.stim_prob(stimopts.positive_stim) = 0.8 ; %positive stim
stimopts.stim_prob(stimopts.neutral_stim) = 0.2 ; %neutral stim
...
if sum(stim_type==stimopts.positive_stim)>0
    outcome_positive_stim(1:floor(stimopts.proba_positive_stim(1)*sum(stim_type==stimopts.positive_stim)),1)=1; %reward
    outcome_positive_stim(end:end+floor(stimopts.proba_positive_stim(3)*sum(stim_type==stimopts.positive_stim)),1)=-1; %punishment
    outcome_positive_stim(end:sum(stim_type==stimopts.positive_stim)) = 0; %nothing
    %randomize
    outcome_positive_stim = outcome_positive_stim(randperm(size(outcome_positive_stim,1),size(outcome_positive_stim,1)));
end

%negative stim
if sum(stim_type==stimopts.negative_stim)>0
    outcome_negative_stim(1:floor(stimopts.proba_negative_stim(1)*sum(stim_type==stimopts.negative_stim)),1)=1; %reward
    outcome_negative_stim(end:end+floor(stimopts.proba_negative_stim(3)*sum(stim_type==stimopts.negative_stim)),1)=-1; %punishment
    outcome_negative_stim(end:sum(stim_type==stimopts.negative_stim)) = 0; %nothing
    %randomize
    outcome_negative_stim = outcome_negative_stim(randperm(size(outcome_negative_stim,1),size(outcome_negative_stim,1)));
end

%Attribute to stim type (match the randomization)
count_p=0;
count_n=0;
stimopts.rewarded_stim=zeros(size(stim_type,1),1);
stimopts.punished_stim=zeros(size(stim_type,1),1);
for i=1:size(stim_type,1)
    if stim_type(i)==stimopts.positive_stim
        count_p=count_p+1;
        if outcome_positive_stim(count_p)==1
            stimopts.rewarded_stim(i)=1;
        elseif outcome_positive_stim(count_p)==-1
            stimopts.punished_stim(i)=1;
        end
    end
    if stim_type(i)==stimopts.negative_stim
        count_n=count_n+1;
        if outcome_negative_stim(count_n)==1
            stimopts.rewarded_stim(i)=1;
        elseif outcome_negative_stim(count_n)==-1
            stimopts.punished_stim(i)=1;
        end
    end
end
...
for i = 1:N
    ...
    digital_out(nidq, nidq_out_list, "trigger", 5/round(app.cur_routine_vals.framerate)) % digital_out(nidq, nidq_out_list, "trigger", 5/round(app.cur_routine_vals.framerate), 4)
    ...
    %Oucome delay: 0.5s fixed for trace conditioning. Might add jitter later
