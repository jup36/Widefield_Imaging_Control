function visual_conditioning_operant_day1_old(app)
%Visualconditioning Imaging Routine Function
% Adapted from visual_sequence by Camden 2019
% Caroline Jahn & Junchol Park (March 2023)function visual_conditioning_operant_day5(app)
%Visualconditioning Imaging Routine Function
% Adapted from visual_sequence by Camden 2019
...
%stim duration (fixed)
stimopts.stim_duration=3; %5; %2 %target duration
...
stimopts.air_puff_duration=0.04;
...
stimopts.stim_prob(stimopts.positive_stim) = 0.5;%75 ; %positive stim
stimopts.stim_prob(stimopts.neutral_stim) = 0; %neutral stim
stimopts.stim_prob(stimopts.negative_stim) = 0.5;%25 ; %negative stim
...
for i = 1:N
    ...
    signal_out = white_noise_only(nidq.Rate, nidq_out_list, 30);
...
%stim duration (fixed)
stimopts.stim_duration=8; %5; %2 %target duration
...
%Dead time durinf which we don't counta lick from presentastio of stim
stimopts.dead_time_lick=0.2; %200ms is the minimum time to process the visual stim, increase this over training
stimopts.time_off_outcome=1; %time between licks that give reward (or airpuff)
...
%Attribute outcome
stimopts.proba_positive_stim = [1 0 0]; % [reward omission punishment]
stimopts.proba_negative_stim = [0 0.5 0.5]; % [reward omission punishment]
...
%% Count the number of rewards received per trial
rewards=zeros(N,1);
stimopts.rewards=rewards;
...
for i = 1:N
    ...
    signal_out = white_noise_only(nidq.Rate, nidq_out_list, total_duration_trial + 1);
    preload(nidq, signal_out)
    start(nidq,"RepeatOutput")
    ...
    while toc <= stimopts.stim_duration
        %monitor the lick port
        if toc > stimopts.dead_time_lick %don't detect licks before dead time (to make sure they see the stim)
            ...
            if lick_logic==1 || switch_logic==1
                ...
                if stimopts.rewarded_stim(i)
                    ...
                    WaitSecs(stimopts.time_off_outcome);
                    fprintf('Water')
                    rewards(i)=rewards(i)+1;
                elseif stimopts.punished_stim(i)
                    %deliver the drop
                    writeDigitalPin(a,airpin,1)
                    WaitSecs(stimopts.air_puff_duration)
    ...
    %wait x sec post stim to make sure we capture whole dynamics of outcome
    %integration
    WaitSecs(stimopts.post_stim_delay);
...
fprintf('Mouse received %d rewards',sum(rewards))
