function visual_conditioning_operant_day6(app)
...
stimopts.stim_prob(stimopts.positive_stim) = 0.5 ; %positive stim
stimopts.stim_prob(stimopts.neutral_stim) = 0 ; %neutral stim DOES NOT WORK WITH DRIFTING !!!!!
stimopts.stim_prob(stimopts.negative_stim) = 0.5 ; %negative stim
...
while size(stim_type,1)<N
    stim_type(end+1) = stimopts.positive_stim; %stimopts.stim_id(randi(length(stimopts.stim_id)));
end
