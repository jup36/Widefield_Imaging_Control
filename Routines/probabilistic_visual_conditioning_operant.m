function probabilistic_visual_conditioning_operant(app)
...
warning('Load arduino sketch lick_switch_water_air_MR_500ms and press Enter to continue')
pause
...
%stim duration (fixed)
stimopts.stim_duration=2; %5; %2 %target duration
...
stimopts.stim_prob(stimopts.positive_stim) = 1;%75 ; %positive stim
stimopts.stim_prob(stimopts.neutral_stim) = 0; %neutral stim
stimopts.stim_prob(stimopts.negative_stim) = 0;%25 ; %negative stim

%Attribute outcome
stimopts.proba_positive_stim = [0.75 0.2 0.05]; % [reward omission punishment]
