function [opts_vars, opts_vals] = resting_state_passive_viewing_config(mouse,experimenter,type)
...
% opts_vars(15) = struct('Name','expose_out_chan','Type','scalar','Values',[0,1,2,3,4,5],'Label','Exposure Out Chan','Editable',1); 
% opts_vars(16) = struct('Name','frame_readout_chan','Type','scalar','Values',[0,1,2,3,4,5],'Label','Frame Readout Chan','Editable',1); 
opts_vars(11) = struct('Name','trigger_ready_chan','Type','scalar','Values',[0,1,2,3,4,5],'Label','Trigger Ready Chan','Editable',1); 
% opts_vars(18) = struct('Name','photodiode_chan','Type','scalar','Values',[0,1,2,3,4,5],'Label','Photodiode Chan','Editable',1); 
% opts_vars(19) = struct('Name','lick_readout_chan','Type','scalar','Values',[0,1,2,3,4,5],'Label','lick_readout_chan','Editable',1); 
% opts_vars(20) = struct('Name','reward_out_chan','Type','scalar','Values',[0,1,2,3,4,5],'Label','reward_out_chan','Editable',1); 

%trial options
opts_vars(12) = struct('Name','number_trials','Type','scalar','Values',[],'Label','Number of Trials','Editable',1); 

%%Define default values 
%General Options
%opts_vals.routine_name='visual_conditioning_with_bhv_cam';
opts_vals.routine_name='resting_state_passive_viewing';

%Imaging Options
opts_vals.exposure_duration = 33.33;  %Camera Exposure in ms
opts_vals.framerate =1000/opts_vals.exposure_duration; %Frame rate
opts_vals.number_trials = 50; %this is the number of sequence of 10 rewards and 10 stimuli being shown
opts_vals.recording_duration=100*opts_vals.number_trials; %Total duration of the recording in seconds.
...
% opts_vals.lick_readout_chan = 4; % 28 on nidq  
% opts_vals.reward_out_chan = 'Port0/Line0'; % 10 on nidq
